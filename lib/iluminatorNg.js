...
module.exports = class Iluminator
{
    ...
    calculateTimes (date, sunCalc, launched, force) {
        ...
        let calculationDate = lodash.clone(date);

        this.currentTime = this.date.getTime();

        calculationDate.setMinutes(this.turnOn[1]);
        calculationDate.setHours(this.turnOn[0]);

        this.onTime = calculationDate.getTime();

        calculationDate.setMinutes(this.minTime[1]);
        calculationDate.setHours(this.minTime[0]);

        this.minimalTime = calculationDate.getTime();

        calculationDate.setMinutes(this.maxTime[1]);
        calculationDate.setHours(this.maxTime[0]);

        this.offTime = calculationDate.getTime();
    ...
    calculateRange () {
        ...
        if (this.sunsetTime >= this.minimalTime) {
