    ...
    if (config.get('app.image_send')) {
        camera.on('read', function (err, timestamp, filename) {
            if (!filename.match(/^[0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2}_[0-9]{4}-[0-9]{1,2}-[0-9]{1,2}_[0-9]+\.jpg~$/)) {
                ...
                log.logDebug('Picture created: ' + path, 'lib', true);
                ...
                request.post(
                    ...
                    function optionalCallback(err, httpResponse, body) {
                        if (err) {
                            log.logError(err, 'lib', true);
                            return console.error('upload failed:', err);
                        }

                        log.logInfo('Upload successful!  Server responded with: ' + body, 'lib');
                    }
                );
            }
        });
...
exports.record = function () {
    ...
    camera = new RaspiCam({
        mode: "video",
        output: path.dirname(__dirname) + '/' + config.get('app.movie_path') + '/' + currentRecord,
